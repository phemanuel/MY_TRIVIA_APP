...
class TriviaTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.db_name = "triviadb_test"
        self.database_path = "postgresql://{}:{}@{}/{}".format(
            "postgres", "root1986", "localhost:5432", self.db_name
        )
        ...
        with self.app.app_context():
            ...
            self.new_question = {'question': 'In what year did Nigeria gain independence?',
                'answer': '1960',
                'category': 4,
                'difficulty': 3,
            }
    ...
    def test_422_if_question_to_delete_Unprocessable(self):
    ...
    def test_add_question_Method_not_allowed(self):
        ...
        res = self.client().post('/questions/1', json=newQuestion)
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 405)
        self.assertEqual(data['success'], False)
        self.assertEqual(data['message'], 'Method not allowed')

    def test_search_if_found(self):
    ...
    def test_search_not_found(self):
        search = {
            'searchTerm': 'whichryryryr ahahahahahah',
        }
        res = self.client().post('/search', json=search)
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 404)
